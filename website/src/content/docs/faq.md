---
title: FAQ
description: Frequently asked questions about Strata.
---

## General

### When should I use Strata instead of etcd?

Use Strata when you want one or more of:
...
- **High write concurrency** — Strata outperforms etcd at 8+ concurrent writers
...
There is no hard limit. Strata scales read throughput linearly with replica count (each follower serves local reads). Write throughput is not affected by replica count — all writes go through the leader.

Odd replica counts aren't required (Strata doesn't use Raft quorum). Any number of followers will work. The leader election mechanism is winner-takes-all via S3 conditional PUT.

### Is Strata production-ready?

Strata has a comprehensive test suite including end-to-end, consistency, stress, failure, and fault injection tests, plus a Jepsen correctness suite. The architecture is designed for simplicity and correctness.
...
### Does Strata require S3 versioning?
...
### How much S3 storage does Strata use?
...
### Does Strata work with Kubernetes?

Yes. The Helm chart deploys Strata as a StatefulSet with persistent volumes, health probes, Prometheus metrics, and optional Envoy proxy for read scale-out.

See the [Kubernetes deployment guide](/deployment/kubernetes/) for details.

### Can Strata replace etcd in a Kubernetes control plane?

Not directly — Kubernetes control planes expect a specific etcd cluster membership API and snapshot restore support that Strata doesn't implement. For embedded Kubernetes (k3s, k0s) using kine as an etcd shim, Strata can serve as the kine backend since kine uses the etcd v3 API.

### How do I expose Strata outside the cluster?
...
### Which etcd client versions work with Strata?
...
Partially. Strata accepts `LeaseGrant` / `LeaseKeepAlive` / `LeaseRevoke` RPCs and assigns lease IDs, but it does **not** evict keys when a lease expires. If your workload depends on lease expiry to clean up keys (e.g. ephemeral service registrations), delete keys explicitly on shutdown instead.
...
A branch is a copy-free fork of a Strata database at a specific checkpoint. It shares SST files with the source database in S3 — no data is copied. Each branch writes its new data to its own S3 prefix.
