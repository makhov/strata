...
func TestNodeScanPagesPrefix(t *testing.T) {
	n := openNode(t)
	c := ctx(t)
	for _, key := range []string{"/scan/a", "/scan/b", "/scan/c", "/skip/a"} {
		if _, err := n.Put(c, key, []byte(key), 0); err != nil {
			t.Fatalf("Put(%q): %v", key, err)
		}
	}
	page, cursor, err := n.Scan(c, t4.ScanOptions{Prefix: "/scan/", Limit: 2})
	if err != nil {
		t.Fatalf("Scan page 1: %v", err)
	}
	if len(page) != 2 || page[0].Key != "/scan/a" || page[1].Key != "/scan/b" {
		t.Fatalf("page 1 = %#v", page)
	}
	page, cursor, err = n.Scan(c, t4.ScanOptions{Prefix: "/scan/", After: cursor, Limit: 2})
	if err != nil {
		t.Fatalf("Scan page 2: %v", err)
	}
	if len(page) != 1 || page[0].Key != "/scan/c" || cursor != "/scan/c" {
		t.Fatalf("page 2 = %#v cursor=%q", page, cursor)
	}
}
